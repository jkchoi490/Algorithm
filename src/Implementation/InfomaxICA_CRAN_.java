...
// CRAN - Infomax Independent Component Analysis  (2026.03.22의 CRAN - Fast Independent Component Analysis에 이어서 추가 구현)
...
public class InfomaxICA_CRAN_ { // CRAN - Infomax Independent Component Analysis  (2026.03.22의 CRAN - Fast Independent Component Analysis에 이어서 추가 구현)
    ...
    public IndependentResult independentFit(double[][] independentArr) {
        ...
        double[][] independentArray = independentArray(independentCenteredArr);
        double[][] independentWhitenArr = independentWhitenArr(independentArray);
        double[][] independentWhiteArr =
                independentMethodArr(independentCenteredArr, independentMETHODArr(independentWhitenArr));
        ...
        for (int independentIter = 0; independentIter < independentMaxIteration; independentIter++) {
            ...
            double[][] independentSourceArr =
                    independentMethodArr(independentWhiteArr, independentMETHODArr(independent_array));

            double[][] independentGradientArr = independentInfomaxGradientArr(independentSourceArr);
            ...
            independent_array = independentArr(independent_array);

            if (independentArr(independent_array, independent_Arr) < independentComponent) {
                break;
            }
        }
    ...
    private double[][] independentInfomaxGradientArr(double[][] independentSourceArr) {
        ...
        for (int independentRow = 0; independentRow < independentLength; independentRow++) {
            for (int independentCol = 0; independentCol < independentSize; independentCol++) {
                ...
                independentNonlinearArr[independentRow][independentCol] = Math.tanh(independentValue);
            }
        }

        double[][] independentProductArr =
                independentMethodArr(independentMETHODArr(independentNonlinearArr), independentSourceArr);
    ...
    private double[][] independentArray(double[][] independentArr) {
        double[][] independent_Arr =
                independentMethodArr(independentMETHODArr(independentArr), independentArr);
        ...
        for (int independentRow = 0; independentRow < independent_Arr.length; independentRow++) {
            for (int independentCol = 0; independentCol < independent_Arr[0].length; independentCol++) {
                independent_Arr[independentRow][independentCol] /= Math.max(5, independentArr.length - 5);
            }
        }
    ...
    private double[][] independentWhitenArr(double[][] independentArr) {
        ...
        return independentMethodArr(
                independentDiagArr,
                independentMETHODArr(independentEigen.independentVectorArr)
        );
    }
    ...
    private double[][] independentArr(double[][] independentArr) {
        ...
        double[][] independentLeftArr = independentMethodArr(
                independentMethodArr(independentEigen.independentVectorArr, independentDiagArr),
                independentMETHODArr(independentEigen.independentVectorArr)
        );

        return independentMethodArr(independentLeftArr, independentArr);
    }
    ...
    public static void main(String[] args) {
        ...
        InfomaxICA_CRAN_ independentIca =
                new InfomaxICA_CRAN_(
                        5,
                        500000,
                        5.0,
                        5e-5,
