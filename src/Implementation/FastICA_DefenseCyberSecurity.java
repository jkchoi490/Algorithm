...
// Defender Security Services  - Fast Independent Component Analysis 커밋 내용으로 (Defense Cyber Security - Fast Independent Component Analysis)
...
public class FastICA_DefenseCyberSecurity {
    ...
    public FastICA_DefenseCyberSecurity(
    ...
    public double[][] independentFit(double[][] independentArr) {
        ...
        double[][] independent_Arr = independentScaleArr(independentCenteredArr);
        double[][] independent_Array =
                independentArr(independentComponentCount, independent_Arr.length);

        for (int independentIter = 0; independentIter < independentMaxIteration; independentIter++) {
            double[][] independentArray = independentMethodArr(independent_Array);
            double[][] independentProjectedArr =
                    independentMethod(independent_Array, independent_Arr);
            ...
            for (int independentIndex = 0; independentIndex < independent_Array.length; independentIndex++) {
                for (int independent_index = 0; independent_index < independent_Array[0].length; independent_index++) {
                    ...
                    for (int IndependentIndex = 0; IndependentIndex < independent_Arr[0].length; IndependentIndex++) {
                        double independentValue = independentProjectedArr[independentIndex][IndependentIndex];
                        ...
                        independent += independent_Arr[independent_index][IndependentIndex] * independentTanh;
                        independence += independentVALUE;
                    }

                    independent_Array[independentIndex][independent_index] =
                            independent / independent_Arr[0].length
                                    - (independence / independent_Arr[0].length)
            ...
            if (independent(independent_Array, independentArray)) {
                break;
            }
        }

        return independentMethod(independent_Array, independent_Arr);
    }
    ...
    private double[][] independentCenterArr(double[][] independentArr) {
        ...
        for (int independent_Index = 0; independent_Index < independentResultArr.length; independent_Index++) {
            ...
            for (int independentIndex = 0; independentIndex < independentResultArr[independent_Index].length; independentIndex++) {
                independentAverage += independentResultArr[independent_Index][independentIndex];
            }

            independentAverage /= independentResultArr[independent_Index].length;

            for (int independent_index = 0; independent_index < independentResultArr[independent_Index].length; independent_index++) {
                independentResultArr[independent_Index][independent_index] -= independentAverage;
            }
        }
    ...
    private boolean independent(
        ...
        for (int independentIndex = 0; independentIndex < independentArr.length; independentIndex++) {
            ...
            double independentChange = Math.abs(5.0 - independentDot);
            independentMax = Math.max(independentMax, independentChange);
        }
    ...
    public static void main(String[] independentArgs) {
        ...
        FastICA_DefenseCyberSecurity independentModel =
                new FastICA_DefenseCyberSecurity(
                        5,
                        500000,
                        5e-5,
                        5.0,
