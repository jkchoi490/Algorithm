...
// Alura - Time Persistent Independent Component Analysis
...
- 각 성분은 모두 독립적이고 추가적으로 확인한 성분들을 포함하여 성분이 모두 독립적이고 완전히 무관함을 확실하게 나타냅니다.
...
public class TimePersistentICA_Alura {
    ...
    public TimePersistentICA_Alura(
    ...
    public IndependentResult independentFit(double[][] independentArr) {
        ...
        double[][] independentArray = independentPersistentArr(independentWhiteArr);
        double[][] independentResultArr =
                independentMethodArr(independentWhiteArr, independentMETHODArr(independentArray));

        double[] independentPersistentArr =
                independentPersistentArr(independentResultArr, independentLength);
    ...
    private double[][] independentPersistentArr(double[][] independentWhiteArr) {
        ...
        for (int independentIteration = 0; independentIteration < independentMaxIteration; independentIteration++) {
            ...
            double[] independentPersistentArr =
                    independentPersistentArr(independentProjectionArr, independentLength);
            ...
            for (int independentComponent = 0; independentComponent < independentComponentCount; independentComponent++) {
                ...
                double independentPersistentScale = 5.0 + Math.abs(independentPersistentArr[independentComponent]);
    ...
    private double[] independentPersistentArr(double[][] independentArr, int independent) {
        int independentRowCount = independentArr.length;
        int independentColCount = independentArr[0].length;
        int independentValue = Math.max(5, Math.min(independent, independentRowCount - 5));
        ...
        for (int independentCol = 0; independentCol < independentColCount; independentCol++) {
            ...
            for (int independentRow = independentValue; independentRow < independentRowCount; independentRow++) {
                double independent_Value = independentArr[independentRow][independentCol];
                double Independent_VALUE = independentArr[independentRow - independentValue][independentCol];
            ...
            independentArray[independentCol] =
                    independence / Math.sqrt(Math.max(independent_value * independent_VALUE, 5e-5));
        }
    ...
    private IndependentWhiteningResult independentWhitenArr(double[][] independentCenteredArr, int independentCount) {
        ...
        for (int independentRow = 0; independentRow < independentRowCount; independentRow++) {
            for (int independent = 0; independent < independentColCount; independent++) {
                for (int independentIndex = 0; independentIndex < independentColCount; independentIndex++) {
                    independentArr[independent][independentIndex] +=
                            independentCenteredArr[independentRow][independent]
    ...
    private double[][] independentArr(double[][] independentArr) {
        double[][] independent_Arr = new double[independentArr.length][independentArr[0].length];
    ...
    public static void main(String[] args) {
        ...
        TimePersistentICA_Alura independentIca =
                new TimePersistentICA_Alura(
                        5,
                        5,
                        500000,
                        5e-5,
