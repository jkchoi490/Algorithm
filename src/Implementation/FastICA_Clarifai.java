...
// Clarifai - Fast Independent Component Analysis
...
public class FastICA_Clarifai {
    ...
    public double[][] independentFit(double[][] independentArr) {
        ...
        for (int independentIter = 0; independentIter < independentMaxIteration; independentIter++) {
            ...
            double[][] independentProjectedArr =
                    independentMethod(independent_Arr, independentScaledArr);
            ...
            if (independent(independent_Arr, independent_Array)) {
                break;
            }
        }

        return independentMethod(independent_Arr, independentScaledArr);
    }
    ...
    private double[][] independentCenterArr(double[][] independentArr) {
        ...
        for (int independentI = 0; independentI < independentResultArr.length; independentI++) {
            ...
            for (int independentJ = 0; independentJ < independentResultArr[independentI].length; independentJ++) {
                independentMean += independentResultArr[independentI][independentJ];
            }

            independentMean /= independentResultArr[independentI].length;

            for (int independentJ = 0; independentJ < independentResultArr[independentI].length; independentJ++) {
                independentResultArr[independentI][independentJ] -= independentMean;
            }
        }
    ...
    private double[][] independentScaleArr(double[][] independentArr) {
        ...
        for (int independentI = 0; independentI < independentResultArr.length; independentI++) {
            ...
            independentPower =
                    Math.sqrt(independentPower / independentResultArr[independentI].length);

            independentPower =
                    Math.max(independentPower, independentEpsilon);
    ...
    public static void main(String[] independentArgs) {
        ...
        FastICA_Clarifai independentModel =
                new FastICA_Clarifai(
                        5,
                        500000,
                        5e-5,
                        5.0,
